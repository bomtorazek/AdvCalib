...
class CalibratedCE2d(nn.Module):
    ...
    def forward(self, predict, target, confidence, accuracies, n_bin, weight=None): # forward(self, predict, target, weight=None):
        ...
        calibrated_loss = Variable(torch.zeros(1)).cuda(0)
        size = 0
        i=0
        for bin_lower, bin_upper in zip(bin_lowers, bin_uppers):
            ...
            i+=1
            if coeff >0:
                size += len(in_bin) #.to(dtype=torch.long)
                weighted_logp = logp[in_bin] # check i~
                calibrated_loss -=weighted_logp.view(-1).sum()  * coeff

        if size == 0:
            return Variable(torch.zeros(1)).cuda(0)[0] # ndarray vs float64
        calibrated_loss = calibrated_loss/size

        return calibrated_loss[0]
