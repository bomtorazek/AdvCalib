    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
                ...
                if USECALI:
                    if (args.lambda_semi > 0 or args.lambda_semi_adv > 0) and i_iter >= args.semi_start_adv:
                        ...
                        if iter_images % train_dataset_size == 0  and iter_images != 0 :
                            ...
                            def eval():
                                loss_cali = nll_criterion(model_cali.temperature_scale(logits), labels_cali)
                                loss_cali.backward()
                                return loss_cali

                            optimizer_cali.step(eval)  # just one backward >> not 50 iterations
                            after_temperature_nll = nll_criterion(model_cali.temperature_scale(logits), labels_cali).item()
                            after_temperature_ece, accuracies, n_bin = ece_criterion(model_cali.temperature_scale(logits), labels_cali)
            ...
            if USED:
                ...
                D_gt_v = Variable(one_hot(labels)).cuda(args.gpu) #one_hot
                ignore_mask_gt = (labels.numpy() == 255) # same as ignore_mask (8,321,321)
